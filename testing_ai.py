...
instruction = (
    ...
    "3. Provide specific unit numbers and calorie-friendly options based on budget."
)
...
while True:
    ...
    with console.status("[bold green]Processing..."):
        try:
            # FIXED: Syntax for chat.send_message in google-genai SDK
            if any(ext in user_input.lower() for ext in [".jpg", ".png"]):
                filename = next((w for w in user_input.split() if "." in w), None)
                if filename and os.path.exists(filename):
                    img = Image.open(filename)
                    # Correct multi-modal format for this SDK:
                    response = chat.send_message(message=[user_input, img])
                else:
